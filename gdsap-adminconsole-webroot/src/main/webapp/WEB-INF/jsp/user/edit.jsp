...
<c:url var="formUrl" value="/user/update"/>
<form method="post" action="${formUrl}" class="required-validate pageForm" onsubmit="return validateCallback(this, navTabAjaxDone);">
	...
	<div class="pageFormContent" layouth="98" style="height: 362px; overflow: auto;">
		...
		<p>
			<a class="button" href="${qubeLoginHost}" target="_blank"><span>Open QUBE</span></a>
		</p>
		...
		<p>
			...
			<input type="text" size="30" name="piLimit" value="${user.piLimit}">
		</p>
		<p>
			<label>User Status</label>
			<select name="userStatus">
				<c:forEach var="src" items="${userStatus}">
					<option value="${src.code}" ${user.userStatus.code == src.code ? 'selected="selected"' : ''}>${src}</option>
				</c:forEach>
			</select>
		</p>
		<p></p>
